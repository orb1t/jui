 ...
 * @since 1.0
 */
public class ButtonLayout implements com.iancaffey.jui.LayoutManager {
    @Override
    public boolean layout(com.iancaffey.jui.Component component) {
        ...
        com.iancaffey.jui.Insets insets = button.getPadding();
        if (insets != null) {
            x += insets.left;
            y += insets.top;
    ...
    @Override
    public Dimension size(com.iancaffey.jui.Component component) {
        if (!(component instanceof Button))
            throw new IllegalArgumentException("Button layout must only be used with buttons.");
        int width = 0;
        int height = 0;
