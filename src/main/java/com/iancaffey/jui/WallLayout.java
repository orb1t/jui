 ...
 * @since 1.0
 */
public class WallLayout implements LayoutManager {
    ...
    public boolean layout(Component component) {
        ...
        Component[] components = container.getComponentArray();
        boolean changed = false;
        int rows = (int) Math.ceil(components.length / 2.0D);
        for (int i = 0; i < rows; i++) {
            int leftIndex = i * 2;
            int currentHeight = 0;
            Component l = components[leftIndex];
            if (l.isVisible()) {
            ...
            if (rightIndex < components.length) {
                Component r = components[rightIndex];
                if (r.isVisible()) {
